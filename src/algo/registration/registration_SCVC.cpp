...
namespace scvc
{
	...
	void SCVC_PSO_Match::Pose_Verfication2(SPose3D_R& resultsS)
	{
		...
		double  thre_a = cos(1.0 / 180.0 * M_PI);
		double  thre_onoverlap_dis = _R_model_para.model.model_samplingdistance *2;
...
#pragma omp parallel for 
		...
		VectorXi target_overlap = VectorXi::Zero(targetrow);
		//VectorXi target_no_overlap = VectorXi::Zero(targetrow);
		VectorXf target_distance = VectorXf::Zero(targetrow);
#ifdef _OPENMP
#pragma omp parallel for 
#endif
		for(int i=0;i< targetrow;i++)
		{
			...
			double target_t_dis = target_t.row(i).norm();
			Vector3f target_t_dir = (target_t.row(i)/ target_t_dis).transpose();
			VectorXf angle = source_t_dir * target_t_dir;
			...
			for (int j = 0; j < 1; j++)
			{
				...
				if (maxs > thre_a)
				{
					double g = _R_model_para.scene.downsample_scene.row(i).tail(3).dot(source.row(indexmaxs).tail(3));
					double d = fabs(target_t_dis - source_t_dis(indexmaxs));
					...
					else if(target_t_dis+ thre_dis >= source_t_dis(indexmaxs))
					{
						//double dmax = d > thre_onoverlap_dis ? d : thre_onoverlap_dis;
						//target_distance(i) = dmax > target_distance(i) ? dmax : target_distance(i);
						//target_distance(i) = thre_onoverlap_dis * (exp(over_thre_a - g) - 1.0) + d;
						target_distance(i) =  d;
						//target_distance(i) = d > thre_onoverlap_dis ? d : thre_onoverlap_dis;
						angle(indexmaxs) = -100;
					}
					else
					{
						//target_no_overlap[i] = 1;
					}
				}
				else
				{
...
#pragma omp parallel for 
			...
			for (int i = 0; i < sourcerow; i++)
			{
				...
				for (int j = 0; j < 1; j++)
				{
					...
					int indexmaxs;
					maxs = angle.maxCoeff(&indexmaxs);
					//if (target_no_overlap[indexmaxs] != 1)
					{
						//angle(indexmaxs) = -100;
						//continue;
					}
					...
					if (maxs > thre_a)
					{
						...
						else if (g > over_thre_a && target_s_dis(indexmaxs) < source_s_dis && target_distance(indexmaxs)< source_s_dis - target_s_dis(indexmaxs))
						{
							
							
							source_distance(i) = source_s_dis - target_s_dis(indexmaxs);
							break;
						}
						else if (source_s_dis + thre_dis >= target_s_dis(indexmaxs))
						{
							//double dmax = d > thre_onoverlap_dis ? d : thre_onoverlap_dis;
							//source_distance(i) = dmax > source_distance(i) ? dmax : source_distance(i);
							//source_distance(i) = thre_onoverlap_dis * (exp(over_thre_a - g) - 1.0) +d
							source_distance(i) = d;
							//target_index(i) = indexmaxs; //测试用，
							//source_distance(i) = d > thre_onoverlap_dis ? d : thre_onoverlap_dis;
							angle(indexmaxs) = -100;
						}
					}
					else
		...
		resultsS.occlude = double(source_distance.sum() + target_distance.sum()) / resultsS.overlap;
		resultsS.overlap = double(source_overlap.sum()+ target_overlap.sum()) / (sourcerow+ targetrow);
		
		cout << resultsS.occlude<<" "<< resultsS.overlap << endl;
		//cout << "overlap ratio= " << float(overlap_n) / row_m << "  occlude ratio=" << distanc / resultsS.overlap << "  distanc=" << distanc << endl;;
		
		//resultsS.occlude = float(occlude_n) / row_m;

	}
	
	void SCVC_PSO_Match::Pose_Verfication3(SPose3D_R& resultsS)
	{
...
#pragma omp parallel for 
		...
		//resultsS.occlude = double(source_distance.sum() + target_distance.sum()) / pow(double(source_overlap.sum() + target_overlap.sum()), 1.0);
		resultsS.overlap = double(source_overlap.sum() + target_overlap.sum()) / (sourcerow + targetrow);
		...
		void SCVC_PSO_Match::Pose_Verfication(SPose3D_R& resultsS)
	{
		...
		double  thre_dis = _R_model_para.model.model_samplingdistance * model_parameter.overlap_dis;//2.0
		{
			...
			resultsS.overlap = float(num);
			resultsS.occlude = -resultsS.overlap;
			//return;

		}
		MatrixXf gtmodel = transformPCPose(_R_model_para.model.downsample_model, resultsS.Pose);

		RowVector3f Sview = resultsS.Pose.block(0, 3, 3, 1).transpose().cast<float>();
		int row_m = gtmodel.rows();
		
		MatrixXf model_m_m = gtmodel.block(0, 0, row_m, 3);
		int row_s = _R_model_para.scene.downsample_scene.rows();
		MatrixXf model_s_m = _R_model_para.scene.downsample_scene.block(0, 0, row_s, 3);

		MatrixXf model_m_s = model_m_m;
		MatrixXf model_s_s = model_s_m;

		VectorXf distance_m_to_m(row_m);
		VectorXf distance_s_to_m(row_s);
		VectorXf distance_m_to_s(row_m);
		VectorXf distance_s_to_s(row_s);

		RowVector3f zero(0, 0, 0);
		for (int i = 0; i < row_m; i++)
		{
			distance_m_to_s(i)=model_m_s.row(i).norm();
			model_m_s.row(i) = model_m_s.row(i) / distance_m_to_s(i);
			if (distance_m_to_s(i) < 0.00001)
			{
				model_m_s.row(i) = zero;
			}
			
			

			RowVector3f n = model_m_m.row(i) - Sview;
			distance_m_to_m(i)= n.norm();
			model_m_m.row(i) = n / distance_m_to_m(i);
			if (distance_m_to_m(i) < 0.00001)
			{
				model_m_m.row(i) = zero;
			}
			
		}
		//std::cout << distance_m_to_s.transpose() << std::endl;
		//std::cout << distance_m_to_m.transpose() << std::endl;
		
		
		for (int i = 0; i < row_s; i++)
		{
			distance_s_to_s(i) = model_s_s.row(i).norm();
			model_s_s.row(i) = model_s_s.row(i) / distance_s_to_s(i);

			if (distance_s_to_s(i) < 0.00001)
			{
				model_s_s.row(i) = zero;
			}

			RowVector3f n = model_s_m.row(i) - Sview;
			distance_s_to_m(i) = n.norm();
			model_s_m.row(i) = n/ distance_s_to_m(i);
			if (distance_s_to_m(i) < 0.00001)
			{
				model_s_m.row(i) = zero;
			}
		}
		//
		//std::cout << distance_s_to_s.transpose() << std::endl;
		//std::cout << distance_s_to_m.transpose() << std::endl;
		//system("pause");
		double  thre_a = cos(1.0 / 180.0 * M_PI);
	
		//double  thre_dis2 = _R_model_para.model.model_samplingdistance * 2.0;
		//从m开始检查，检查s得遮挡和重叠度
		int overlap_n=0;
		int occlude_n=0;
		int disnum = 0;
		...
		for (int i = 0; i < row_m; i++)
		{
			Vector3f mn = model_m_s.row(i).transpose();
			//作差求余弦夹角
			//std::cout << mn << std::endl;
			VectorXf acosms = model_s_s * mn;
			//std::cout << model_s_s.block(0,0,10,3) << std::endl;
			float max;
			int indexmax;
			max = acosms.maxCoeff(&indexmax);
			if (!isfinite(max))
			{
			...
			//std::cout << acosms.transpose() << std::endl;
			//std::cout << model_s_s.row(indexmax) << std::endl;
			//std::cout << model_m_s.row(i) << std::endl;
			//system("pause");
			if (isfinite(max) && max >= thre_a)//线上有点
			{
				float dis = fabs(distance_m_to_s(i) - distance_s_to_s(indexmax));
				
				
				if (dis < thre_dis)//重叠
				{
					double g = gtmodel.row(i).tail(3).dot(
						_R_model_para.scene.downsample_scene.row(indexmax).tail(3));
					if (g > over_thre_a)
					{
						overlap_n++;
				...
				{
					if (distance_m_to_s(i) < distance_s_to_s(indexmax))//m遮挡s
					{
						occlude_n++;
					...
					{
						//从m视角观察
						Vector3f mm = model_m_m.row(i).transpose();
						VectorXf acossm = model_s_m * mm;

						float maxs;
						int indexmaxs;
						maxs = acossm.maxCoeff(&indexmaxs);
						
						if (maxs > thre_a) //&& distance_s_to_m(indexmaxs) < distance_m_to_m(i)- thre_dis2)//可以gan
						{
							float dis = fabs(distance_m_to_m(i) - distance_s_to_m(indexmaxs));
							if (dis < thre_dis)//重叠
							{
								double g = gtmodel.row(i).tail(3).dot(
									_R_model_para.scene.downsample_scene.row(indexmaxs).tail(3));
								if (g > over_thre_a)
								{
									overlap_n++;
								}
							...
							else if (distance_s_to_m(indexmaxs) < distance_m_to_m(i))
							{
								distanc += dis;
								disnum++;
							}
			...
			else//线上没有点
			{
				...
				if (isfinite(maxs) && maxs > thre_a)
				{
					float dis = fabs(distance_s_to_m(indexmaxs) - distance_m_to_m(i));
					if (dis < thre_dis)//重叠
					{
						double g = gtmodel.row(i).tail(3).dot(
							_R_model_para.scene.downsample_scene.row(indexmaxs).tail(3));
					...
					else if(distance_s_to_m(indexmaxs) < distance_m_to_m(i))//可以gan
					{
						distanc += distance_m_to_m(i) - distance_s_to_m(indexmaxs);
						disnum++;
						occlude_n++;
					}
				}
		...
		if (row_m < row_s)
			row_m = row_s;
		//cout << "overlap ratio= " << float(overlap_n) / row_m << "  occlude ratio=" << distanc / resultsS.overlap << "  distanc=" << distanc << endl;;
		resultsS.occlude = distanc / resultsS.overlap;
		resultsS.overlap = float(overlap_n) / row_m;
		//resultsS.occlude = float(occlude_n) / row_m;
		
}
	void  SCVC_PSO_Match::Model_Scene_down(const MatrixXf& original_model, const MatrixXf& original_scene)
	{
		...
		Vector2f s_xRange, s_yRange, s_zRange;
		computeBboxStd(original_scene, s_xRange, s_yRange, s_zRange); //验证

		//计算包围盒直径
		dx = s_xRange[1] - s_xRange[0];
		dy = s_yRange[1] - s_yRange[0];
		dz = s_zRange[1] - s_zRange[0];
		//场景直径
		_R_model_para.scene.scene_diameter = sqrt(dx * dx + dy * dy + dz * dz);
		//保存场景
		_R_model_para.scene.original_scene = original_scene;

		if (original_scene.rows()< original_model.rows())
		{
			double min = 0.5;
			double max = 0.01;
			int pnum = model_parameter.relativeSamplingnum;//绝对值
			int n = 100;
			_R_model_para.model.model_samplingdistance = 0.05;
			while (n--)
			{
				_R_model_para.scene.downsample_scene = samplePCByQuantization(original_scene, s_xRange, s_yRange, s_zRange, _R_model_para.model.model_samplingdistance, 0);
				if (fabs(_R_model_para.scene.downsample_scene.rows() - pnum) < 500)
				{
					break;
				}
				if (_R_model_para.scene.downsample_scene.rows() > pnum)
				{
				...
				cout << _R_model_para.scene.downsample_scene.rows() << endl;
				//system("pause");
			}
			...
			min = 0.5;
			max = 0.001;
			pnum = model_parameter.SamplingNumber_f;
			n = 100;
			_R_model_para.model.model_feat_samplingdistance = 0.05;
			while (n--)
			{
				_R_model_para.scene.downsample_scene_f= samplePCByQuantization(original_scene, s_xRange, s_yRange, s_zRange,
					_R_model_para.model.model_feat_samplingdistance, 0);
				if (fabs(_R_model_para.scene.downsample_scene_f.rows() - pnum) < 500)
				{
					break;
				}
				if (_R_model_para.scene.downsample_scene_f.rows() > pnum)
				{
					max = _R_model_para.model.model_feat_samplingdistance / _R_model_para.model.model_diameter;
					_R_model_para.model.model_feat_samplingdistance = _R_model_para.model.model_diameter * (max + min) / 2.0;
				}
				else
				{
					min = _R_model_para.model.model_feat_samplingdistance / _R_model_para.model.model_diameter;
					_R_model_para.model.model_feat_samplingdistance = _R_model_para.model.model_diameter * (max + min) / 2.0;
				}
			}

			{
				...
				//场景下采样
				_R_model_para.model.downsample_model = samplePCByQuantization(original_model, xRange, yRange, zRange, SampleStepdistance, 0);
				_R_model_para.model.downsample_model_f = samplePCByQuantization(original_model, xRange, yRange, zRange,
					_R_model_para.model.model_feat_samplingdistance, 0);
			}
		}
		else
		{
			...
			while (n--)
			{
				_R_model_para.model.downsample_model = samplePCByQuantization(original_model, xRange, yRange, zRange, _R_model_para.model.model_samplingdistance, 0);
			...
			while (n--)
			{
				...
				if (fabs(_R_model_para.model.downsample_model_f.rows() - pnum) < 500)
				{
					break;
				}
				if (_R_model_para.model.downsample_model_f.rows() > pnum)
				{
					max = _R_model_para.model.model_feat_samplingdistance / _R_model_para.model.model_diameter;
					_R_model_para.model.model_feat_samplingdistance = _R_model_para.model.model_diameter * (max + min) / 2.0;
				}
...

			{
				...
				//场景下采样
				_R_model_para.scene.downsample_scene = samplePCByQuantization(original_scene, s_xRange, s_yRange, s_zRange, SampleStepdistance, 0);
				_R_model_para.scene.downsample_scene_f = samplePCByQuantization(original_scene, s_xRange, s_yRange, s_zRange,
					_R_model_para.model.model_feat_samplingdistance, 0);
				//_R_model_para.scene.downsample_scene = samplePCByQuantization_normal(original_scene, xRange, yRange, zRange, (float)SampleStepdistance, model_parameter.Sampling_threshold_Angles / 180 * 3.1415, 1, sumnum_scene);//无加权点云下采样
			}
		}
		std::cout << "模型点数：" << _R_model_para.model.downsample_model.rows() << std::endl;
		std::cout << "特征模型点数：" << _R_model_para.model.downsample_model_f.rows() << std::endl;

		std::cout << "场景点数=" << _R_model_para.scene.downsample_scene.rows() << endl;
		std::cout << "特征场景点数=" << _R_model_para.scene.downsample_scene_f.rows() << endl;
	...
	void  SCVC_PSO_Match::Modeldown(const MatrixXf& original_model, int const feature_rows)
	{
...

		{
			...
			//system("pause");
			//clock_t tick1 = clock();
			//MatrixXf pc = sample_FSP(_R_model_para.model.original_model, pnum);
			//clock_t tick2 = clock();
			//writePLY(pc, "E:/modelf.ply");
			//cout << (double)(tick2 - tick1) / CLOCKS_PER_SEC << endl;
			//system("pause");
			while (n--)
			{
			...
			else
			{
				...
				while (n--)
				{
					_R_model_para.model.downsample_model_f = samplePCByQuantization(_R_model_para.model.original_model, xRange, yRange, zRange,
						_R_model_para.model.model_feat_samplingdistance, 0);
					//_R_model_para.model.downsample_model = samplePCByQuantization_normal(_R_model_para.model.original_model,
					//	xRangelist[0], yRangelist[0], zRangelist[0],
					//	(float)_R_model_para.model.model_samplingdistance,
	...
	void SCVC_PSO_Match::trainModel_SCVC_R()
	{
		...
		_R_model_para.model.hash_table.clear();
		{
			...
			int  downsample_model_rows = _R_model_para.model.downsample_model.rows();
			int corres_size = Correspondencepoint.size();
			int size = corres_size * downsample_model_rows;//哈希表大小

			//相关数据初始化
			_R_model_para.model.hash_table.resize(size);//建立哈希表
			_R_model_para.model.hash_nodes = (THash_R*)calloc(size, sizeof(THash_R));
			...
			//double radiusmax = _R_model_para.model.model_diameter * 0.7;
			//构建SCVC特征哈希表;
			double angle_step_radians = M_2PI / model_parameter.SCVC_anglenumber;
			...
			for (int p = 0; p < corres_size; p++)
			{
				...
				const Vector3f p1 = _R_model_para.model.downsample_model.row(i).head(3);//获得坐标
				const Vector3f n1 = _R_model_para.model.downsample_model.row(i).tail(3);//获得法向

				for (int j = 0; j < downsample_model_rows; j++)
				{
					if (i != j)
					{
						...
						computeSCVCFeatures(p1, n1, p2, n2, f);//计算特征值f
						//对一些不符合常规的特征去掉，主要为平面上奇异性大的点对
						if (f[3] > model_minlenght && f[2] > t1 && (f[0] > t2 || f[0] < t3))//	&& f[3] <radiusmax
						{
							//fi[0] = int(f[0] / angle_step_radians);
							//fi[1] = int(f[1] / angle_step_radians);
							//fi[2] = int(f[2] / angle_step_radians);
							//fi[3] = int(f[3] / model_ppf_distance_step);
							...
							uint ppfInd = p * downsample_model_rows + j;

							KeyType hashValue = hashkey(nn)% (_R_model_para.model.hash_table.size());//计算哈希表位置

							//获得属性表的记录位置
							THash_R* hashNode = &(_R_model_para.model.hash_nodes[ppfInd]);
							//cout << hashValue << endl;
							hashNode->is = i;
							hashNode->it = j;
							...
							//插入结点
							_R_model_para.model.hash_table[hashValue].index.push_back(ppfInd);
						}
					}
				}
			}
	...
	void SCVC_PSO_Match::FeatureMatch(const MatrixXf& feature_model, const MatrixXf& feature_scene)
	{
		...
		for (size_t i = 0; i < model_row; ++i)
		{
			...
			if (isfinite(scene_dists[i][0]) && isfinite(model_dists[scene_indices[i][0]][0]) && i == model_indices[scene_indices[i][0]][0])
			{
				...
				//double a1 = feature_cos(RowVectorXf(feature_scene.row(scene_indices[i][0])),
				//	RowVectorXf(feature_scene.row(scene_indices[i][1])));
				//double a2 = feature_cos(RowVectorXf(feature_model.row(model_indices[scene_indices[i][0]][0])),
				//	RowVectorXf(feature_model.row(model_indices[scene_indices[i][0]][1])));//a.value = feature_cos(source_features->at(i), target_features->at(neighbors[0]));
				a.value = a1;
		...
		/*for (size_t i = 0; i < Correspondencepoint.size(); ++i)
		{
			std::cout << Correspondencepoint[i].model << " " << Correspondencepoint[i].scene
				<< " " << Correspondencepoint[i].value << std::endl;

		}*/

		std::cout << "特征对1 " << Correspondencepoint.size() << std::endl;
		std::cout << "特征对2 " << noCorrespondencepoint.size() << std::endl;
		int corresnum = 1000;
		...
		//writePLY(model_f, "modelf.ply");
		//writePLY(scene_f, "scenef.ply");
		...
		//writePLY(model, "model.ply");
		//writePLY(scene, "scene.ply");
	}
...
#pragma omp parallel for 
		...
		std::cout << "评价特征 " << _R_model_para.model.feature_point.rows() << std::endl;
		std::cout << "特征对1 " << Correspondencepoint.size() << std::endl;
		std::cout << "特征对2 " << noCorrespondencepoint.size() << std::endl;
...
#pragma omp parallel for 
		...
		//writePLY(scene_f, "scenef.ply");
	    /* {
			int row = _R_model_para.model.downsample_model.rows();
			KDTree* kdt1 = BuildKDTree(model_f.block(0,0, corresnum,3));
			std::vector<std::vector<int>> indices;
			std::vector<std::vector<float>> dists;
			SearchKDTree(kdt1, _R_model_para.model.downsample_model.block(0, 0, row, 3), indices, dists, 1);
			delete kdt1;
			
			double d= _R_model_para.model.model_samplingdistance * _R_model_para.model.model_samplingdistance*0.36;
			...
			for (int i = 0; i < row; i++)//
			{
				if (dists[i][0]>=d)
				{
					_R_model_para.model.downsample_model.row(rown)
						= _R_model_para.model.downsample_model.row(i);
					rown++;
				}
			}
			_R_model_para.model.downsample_model.conservativeResize(rown, 6);
			//cout << row << " " << rown << endl;
		}


		{
			int row2 = _R_model_para.scene.downsample_scene.rows();
			KDTree* kdt1 = BuildKDTree(scene_f.block(0, 0, corresnum, 3));
			std::vector<std::vector<int>> indices;
			std::vector<std::vector<float>> dists;
			SearchKDTree(kdt1, _R_model_para.scene.downsample_scene.block(0, 0, row2, 3), indices, dists, 1);
			delete kdt1;
			...
			for (int i = 0; i < row2; i++)//
			{
				if (dists[i][0] >= d)
				{
					_R_model_para.scene.downsample_scene.row(rown)
						= _R_model_para.scene.downsample_scene.row(i);
					rown++;
				}
				
			}
			_R_model_para.scene.downsample_scene.conservativeResize(rown, 6);
			//cout << row2 << " " << rown << endl;
	...
	void SCVC_PSO_Match::FeatureMatch()
	{
		...
		model_features = source_features;
		scene_features = target_features;
		...
		double feature_radius = _R_model_para.model.model_feat_samplingdistance*6;
		int scene_row = _R_model_para.scene.downsample_scene_f.rows();
		int model_row = _R_model_para.model.downsample_model_f.rows();
...
#pragma omp parallel for 
		...
		for (int i = 0; i < model_row; i++)
		{
			source_xyz->points[i].x = _R_model_para.model.downsample_model_f(i, 0);
			source_xyz->points[i].y = _R_model_para.model.downsample_model_f(i, 1);
			source_xyz->points[i].z = _R_model_para.model.downsample_model_f(i, 2);

			source_normal->points[i].normal_x = _R_model_para.model.downsample_model_f(i, 3);
			source_normal->points[i].normal_y = _R_model_para.model.downsample_model_f(i, 4);
			source_normal->points[i].normal_z = _R_model_para.model.downsample_model_f(i, 5);
		}
		...
		source_xyz.~shared_ptr();
		source_normal.~shared_ptr();

		target_xyz->points.resize(scene_row);
		target_normal->points.resize(scene_row);
#ifdef _OPENMP
#pragma omp parallel for 
#endif
		for (int i = 0; i < scene_row; i++)
		{
			target_xyz->points[i].x = _R_model_para.scene.downsample_scene_f(i, 0);
			target_xyz->points[i].y = _R_model_para.scene.downsample_scene_f(i, 1);
			target_xyz->points[i].z = _R_model_para.scene.downsample_scene_f(i, 2);

			target_normal->points[i].normal_x = _R_model_para.scene.downsample_scene_f(i, 3);
			target_normal->points[i].normal_y = _R_model_para.scene.downsample_scene_f(i, 4);
			target_normal->points[i].normal_z = _R_model_para.scene.downsample_scene_f(i, 5);
		}
		...
		target_xyz.~shared_ptr();
		target_normal.~shared_ptr();
		searchMothodPtr.~shared_ptr();

		pcl::KdTreeFLANN<pcl::FPFHSignature33> scene_matching;
		scene_matching.setInputCloud(target_features);

		pcl::KdTreeFLANN<pcl::FPFHSignature33> model_matching;
		model_matching.setInputCloud(source_features);
...
#pragma omp parallel for 
		...
		for (int i = 0; i < model_row; ++i)
		{
			...
			// Ignore NaNs.
			if (isfinite(source_features->at(i).histogram[0]))
			{
				// Find the nearest neighbor (in descriptor space)...
				int neighborCount = scene_matching.nearestKSearch(source_features->at(i), 2, neighbors, squaredDistances);
				// ...and add a new correspondence if the distance is less than a threshold
				// (SHOT distances are between 0 and 1, other descriptors use different metrics).
				if (neighborCount == 2)
				{
					if (isfinite(target_features->at(neighbors[0]).histogram[0]))
					{
						std::vector<int> s_neighbors(2);
						int neighborCount = model_matching.nearestKSearch(target_features->at(neighbors[0]), 2, s_neighbors, squaredDistances);

						if (neighborCount == 2 && s_neighbors[0]== i)
						{
							Correspondence a;
							a.model = i;
							a.scene = neighbors[0];
							...
							double a1 = feature_cos(source_features->at(i), target_features->at(neighbors[1]));
							double a2= feature_cos(source_features->at(i), target_features->at(neighbors[0]));
							//a.value = feature_cos(source_features->at(i), target_features->at(neighbors[0]));
							a.value =  a1 / (a2 + 0.00001);
							Correspondencepoint[i]=a;
							noCorrespondencepoint[i].value = -1;
						}
						else
		...
		std::cout << "特征点对2 " << corresnum << std::endl;
...
#pragma omp parallel for 
	...
	double SCVC_PSO_Match::feature_cos(pcl::FPFHSignature33 &a, pcl::FPFHSignature33 &b)
	{
		double a1, a2, a3;
		a1 = 0;
		a2 = 0;
		...
		for (int k = 0; k < 33; k++)
		{
			a1 += a.histogram[k]* b.histogram[k];
			a2 += a.histogram[k]* a.histogram[k];
			a3 += b.histogram[k]* b.histogram[k];
		}
		if (a1 > 0.00001 && a3 > 0.00001)
		{
			a1= a1 / sqrt(a2) / sqrt(a3);
		}
		else
		{
			a1 = 0;
	...
	void SCVC_PSO_Match::match_SCVC_registration2(std::vector<SPose3D_R>& resultsS,const ppf_scenematching_parameter parameter)
	{
		...
		// 清楚前一次模型哈希表的数据
			...
			std::cout << "model_parameter.SCVC_distance(绝对): " << model_ppf_distance_step << endl;
			std::cout << "model_parameter.SCVC_anglenumber: " << model_parameter.SCVC_anglenumber << endl;
			double angle_step_radians = M_2PI / model_parameter.SCVC_anglenumber;
...
#pragma omp parallel for 
			...
			for (int p = 0; p < corres_size; p++)
			{
				...
				//投票
				{
					...
					for (int jj = 0; jj < scenerows; jj++)
					{
						...
						computeSCVCFeatures(sp1, sn1, sp2, sn2, f);//计算特征值f
			...
			if (resultsS3.size() > model_parameter.voting_top)//默认200
...
#pragma omp parallel for 
			...
			for (int r = 0; r < resultsS3.size(); r++)
			{
				...
				if(parameter.ICP_nbIterations>0)
				   Optimizing_registration(resultsS3[r], parameter.ICP_nbIterations, true);
				//cout << "聚类前获得姿态数1：\n" << Tmg << endl;
			}

		clock_t tick2 = clock();
		//cout << (double)(tick4 - tick3) / CLOCKS_PER_SEC << " sec  pose......" << endl;
		std::cout << "聚类前获得姿态数：" << resultsS3.size() << endl;
		clock_t tick3 = clock();
		if (resultsS3.size() > 1)
		{
...
#pragma omp parallel for 
				...
				for (int p = 0; p < resultsS3.size(); p++)
				{
					//验证姿态
					if (parameter.ICP_nbIterations > 0)
					     Optimizing_registration(resultsS3[p], parameter.ICP_nbIterations, true);
				}
				{
					int p = 0;
					Pose_Verfication3(resultsS3[p]);
					//cout << resultsS3[r].numVotes << "   cc  " << resultsS3[r].overlap << "   cc  " << resultsS3[r].occlude << endl;

					Matrix4d dp = resultsS3[p].Pose *groundtruth_pose .inverse();
					double d = (dp.block(0, 0, 3, 3).trace() - 1.0) / 2.0;
					if (d > 1) d = 1;
					if (d < -1)    d = -1;
					acos(d);

					double dis = (groundtruth_pose.block(0, 3, 3, 1) - resultsS3[p].Pose.block(0, 3, 3, 1)).norm();
		...
		{
			std::cout << "获得姿态数：" << resultsS3.size() << " " << resultsS3[0].numVotes << endl;
	...
	void SCVC_PSO_Match::computeSCVCFeatures(const Vector3f& p1, const Vector3f& n1,
	...
	void  SCVC_PSO_Match::Optimizing_registration(SPose3D_R& resultsS, int max_iter,bool down)
	{
		...
		double max_dis = _R_model_para.model.model_samplingdistance * 10.0;//20
		if (!down)
			max_dis = _R_model_para.model.model_samplingdistance * 10.0;//20
		...
		//double  over_thre_a = cos(model_parameter.overlap_angle / 180.0 * M_PI);//40
		//&& model.row(i).tail(3).dot(scene.row(indices[i][j]).tail(3)) > over_thre_a
        //&& model.row(i).tail(3).dot(scene.row(indices[i][0]).tail(3)) > over_thre_a
		...
		int  srow = scene.rows();
		//cout << scene.rows() <<"  " << model.rows() << endl;
		//cout << sceneindex.sum() << " sceneindex.sum() " << modelindex.sum() << endl;
	...
	Matrix4d   SCVC_PSO_Match::SparceICP_Optimizing(MatrixXf source, MatrixXf target, int nbIterations)
	{
		
		/*int nbIterationsIn = nbIterations;
		double mu = 8;
		int nbIterShrink = 5;
		double p = 0.5;
		...
		IcpOptimizer myIcpOptimizer(source.cast<double>(), target.cast<double>(), kNormals, nbIterations, nbIterationsIn, mu, nbIterShrink, p, method, verbose, 2);
	...
	Matrix4d   SCVC_PSO_Match::ICP_Optimizing(MatrixXf source, MatrixXf target, int max_iter)
	{
		...
		//MatrixXf pct = transformPCPose(model_para.model[model_id].downsample_model, resultsS.Pose);//模型1
		//MatrixXf pct1 = model_para.scene[model_id].downsample_scene
