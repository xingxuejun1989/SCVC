...
#pragma omp for
		...
		int verbose, ICP_xform_type xform_type)
	{
		...
		if (median_dist < 0.000001)
			return -1.0;
		float dist2_thresh = sqr(maxdist);
		angle_thresh = clamp(ANGLE_THRESH_MULT * median_angle, ANGLE_THRESH_MIN, ANGLE_THRESH_MAX);
		float normdot_thresh = cos(angle_thresh);
		...
		for (size_t i = 0; i < npairs; i++)
		{
			if (distances2[i] > dist2_thresh ||	normdots[i] < normdot_thresh)
				continue;
			pairs[next++] = pairs[i];
		}
		//cout << next << " " << pairs.size() << endl;
		pairs.erase(pairs.begin() + next, pairs.end());

		//cout<< next<<" xx "<< median_dist <<endl;
		timestamp t3 = now();
		if (verbose > 1)
		{
			dprintf("Rejected %lu pairs in %.3f msec.\n", 	(unsigned long)(npairs - pairs.size()), (t3 - t2) * 1000.0f);
		}
		...
		for (int i = 0; i < nv1; i++) 
		{
			...
			for (int j = 0; j < 3; j++)
				for (int k = j; k < 3; k++)
					A1[j][k] += mesh1->normals[i][j] *	mesh1->normals[i][k];
		}
		for (int i = 0; i < nv2; i++) 
		{
			if (!weights2[i])
		...
		for (int i = 0; i < nv1; i++) 
		{
			...
			float s = 0.0f;
			for (int j = 0; j < 3; j++)
				s += eval1[j] * sqr(A1[0][j] * mesh1->normals[i][0] +	A1[1][j] * mesh1->normals[i][1] + 	A1[2][j] * mesh1->normals[i][2]);
	...
	static void make_uniform_cdfs(const vector<float> &weights1, vector<float> &sampcdf1, const vector<float> &weights2, vector<float> &sampcdf2)
	{
	...
	float ICP(TriMesh *mesh1, TriMesh *mesh2, const xform &xf1, xform &xf2, const KDtree *kd1, const KDtree *kd2, vector<float> &weights1, vector<float> &weights2, float maxdist /* = 0.0f */, int verbose /* = 0 */,int max_iter/* = 150 */, ICP_xform_type xform_type /* = ICP_RIGID */)
	{
		...
		if (maxdist <= 0.0f)
		{
			...
			//cout << xf1 << endl;
			//cout << xf2 << endl;
			//cout << mesh1->bsphere.center << endl;
			//cout << mesh2->bsphere.center << endl;
			//cout << xf1 * mesh1->bsphere.center << endl;
			//cout << xf2 * mesh2->bsphere.center << endl;
			double d = maxdist + fabs(mesh1->bsphere.r - mesh2->bsphere.r);
			//cout << d << endl;
			if (d < 0.000001)
			{
				return -1.0;
		...
		ICP_xform_type iter_xform_type = (xform_type == ICP_TRANSLATION) ? ICP_TRANSLATION : ICP_RIGID;
		...
		for (iter = 0; iter < max_iter; iter++)
		{
			...
			if (recompute)
			{
				if (!had_weights)
				{
					compute_overlaps(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2, maxdist, verbose);
				}
			...
			// Do an iteration
			err = ICP_iter(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2, sampcdf1, sampcdf2, DESIRED_PAIRS, cdfincr, recompute, maxdist, angle_thresh, verbose, iter_xform_type);
			//cout<<"i== "<< iter<<"  " << err << endl;
			...
			if (iter - iter_of_min_err >= TERMINATION_ITER_THRESH && iter_of_min_err >= 0 && !recompute &&	xform_type != ICP_SIMILARITY && xform_type != ICP_AFFINE)
			{
			...
			if (iter == max_iter / 2 && (xform_type == ICP_SIMILARITY || xform_type == ICP_AFFINE))
		...
		for (iter = 0; iter < FINAL_ITERS; iter++)
		{
			...
			err = ICP_iter(mesh1, mesh2, xf1, xf2, kd1, kd2, weights1, weights2, sampcdf1, sampcdf2, DESIRED_PAIRS_FINAL, cdfincr, false, maxdist, angle_thresh, verbose, iter_xform_type);
			if (err < 0)
			{
				if (!had_weights)
				{
	...
	// Do ICP.  Aligns mesh mesh2 to mesh1, updating xf2 with the new transform.
	// Returns alignment error, or -1 on failure
	float ICP(TriMesh *mesh1, TriMesh *mesh2,	const xform &xf1, xform &xf2,	const KDtree *kd1, const KDtree *kd2, vector<float> &weights1, vector<float> &weights2,	float maxdist /* = 0.0f */, int verbose /* = 0 */,	ICP_xform_type xform_type /* = ICP_RIGID */)
	{
		timestamp t_start = now();
		...
		for (iter = 0; iter < MAX_ITERS; iter++) 
		{
			...
			// If we're optimizing for similarity or affine, switch on
			// those transformations after MAX_ITERS/2 iterations
			if (iter == MAX_ITERS / 2 &&	(xform_type == ICP_SIMILARITY || xform_type == ICP_AFFINE))
				iter_xform_type = xform_type;
		}
		cout << "iter = " << iter << endl;
		//cout << "err1 = " << err << endl;
	...
	float ICP(TriMesh *mesh1, TriMesh *mesh2,	const xform &xf1, xform &xf2,	const KDtree *kd1, const KDtree *kd2, int verbose /* = 0 */, ICP_xform_type xform_type /* = ICP_RIGID */)
	{
	...
	float ICP(TriMesh *mesh1, TriMesh *mesh2,	const xform &xf1, xform &xf2,	int verbose /* = 0 */, ICP_xform_type xform_type /* = ICP_RIGID */)
	{
		if (verbose > 1)
			dprintf("\nBuilding KDtrees... ");
		timestamp t = now();
...
#pragma omp parallel for
	...
	float ICP(TriMesh *mesh1, TriMesh *mesh2,	const xform &xf1, xform &xf2,	const KDtree *kd1, const KDtree *kd2,::std::vector<float> &weights1, ::std::vector<float> &weights2,	float maxdist, int verbose, bool do_scale, bool do_affine)
	{
		ICP_xform_type xform_type = do_affine ? ICP_AFFINE :
			do_scale ? ICP_SIMILARITY : ICP_RIGID;
		return ICP(mesh1, mesh2, xf1, xf2, kd1, kd2,	weights1, weights2, maxdist, verbose, xform_type);
	}

	float ICP(TriMesh *mesh1, TriMesh *mesh2, const xform &xf1, xform &xf2,	const KDtree *kd1, const KDtree *kd2,	int verbose, bool do_scale, bool do_affine)
	{
		ICP_xform_type xform_type = do_affine ? ICP_AFFINE :
			do_scale ? ICP_SIMILARITY : ICP_RIGID;
	...
	float ICP(TriMesh *mesh1, TriMesh *mesh2, const xform &xf1, xform &xf2,	int verbose, bool do_scale, bool do_affine)
	{
		ICP_xform_type xform_type = do_affine ? ICP_AFFINE :
			do_scale ? ICP_SIMILARITY : ICP_RIGID;
